import { CloudUpload, Loader2 } from "lucide-react";
import { DragEvent, useRef, useState } from "react";
import { Trans, useTranslation } from "react-i18next";
import { clsx } from "../utils/clsx";

interface Props {
  accept?: string;
  multiple?: boolean;
  onFiles: (files: File[]) => void;
  hint?: string;
  disabled?: boolean;
  /** When true, replaces the prompt with a spinner + processing label.
   *  The server can take 3–4 seconds per book parsing the EPUB, extracting
   *  text, and initializing the editor repo — without this indicator the UI
   *  looks frozen. */
  uploading?: boolean;
  /** Number of files currently being processed. Shown in the busy label. */
  uploadingCount?: number;
}

export function Dropzone({
  accept = ".epub",
  multiple = true,
  onFiles,
  hint,
  disabled,
  uploading = false,
  uploadingCount = 0,
}: Props) {
  const { t } = useTranslation();
  const inputRef = useRef<HTMLInputElement>(null);
  const [drag, setDrag] = useState(false);

  // Treat uploading as a kind of disabled — block drops, suppress hover/drag
  // styling, swap content for the busy label.
  const inert = disabled || uploading;

  function handle(files: FileList | null) {
    if (!files || files.length === 0) return;
    onFiles(Array.from(files));
  }
  function onDrop(e: DragEvent<HTMLLabelElement>) {
    e.preventDefault(); setDrag(false);
    if (inert) return;
    handle(e.dataTransfer.files);
  }

  return (
    <label
      onDragOver={(e) => { e.preventDefault(); if (!inert) setDrag(true); }}
      onDragLeave={() => setDrag(false)}
      onDrop={onDrop}
      aria-busy={uploading || undefined}
      className={clsx(
        "flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-colors sm:px-10 sm:py-12",
        drag && !inert
          ? "border-stone-700 bg-stone-100 dark:border-stone-300 dark:bg-stone-800"
          : uploading
            ? "border-stone-400 bg-stone-50 dark:border-stone-500 dark:bg-stone-800/60"
            : "border-stone-300 hover:border-stone-500 hover:bg-stone-50 dark:border-stone-700 dark:hover:border-stone-500 dark:hover:bg-stone-800/40",
        inert && "cursor-not-allowed",
        disabled && !uploading && "opacity-50",
      )}
    >
      <div className="grid h-12 w-12 place-items-center rounded-full bg-stone-100 ring-1 ring-stone-200 dark:bg-stone-800 dark:ring-stone-700">
        {uploading
          ? <Loader2 className="h-5 w-5 animate-spin text-stone-500 dark:text-stone-400" strokeWidth={1.75} />
          : <CloudUpload className="h-5 w-5 text-stone-500 dark:text-stone-400" strokeWidth={1.75} />}
      </div>
      <div className="space-y-1">
        {uploading ? (
          <>
            <div className="text-sm font-medium text-stone-700 dark:text-stone-200">
              {t("dropzone.processing", { count: Math.max(uploadingCount, 1) })}
            </div>
            <div className="text-xs text-stone-500 dark:text-stone-500">
              {t("dropzone.processingHint")}
            </div>
          </>
        ) : (
          <>
            <div className="text-sm font-medium text-stone-700 dark:text-stone-200">
              <Trans
                i18nKey="dropzone.prompt"
                t={t}
                components={{
                  browse: <span className="underline underline-offset-2 decoration-stone-400" />,
                }}
              />
            </div>
            {hint && <div className="text-xs text-stone-500 dark:text-stone-500">{hint}</div>}
          </>
        )}
      </div>
      <input ref={inputRef} type="file" className="sr-only" accept={accept} multiple={multiple}
        disabled={inert} onChange={(e) => handle(e.target.files)} />
    </label>
  );
}
