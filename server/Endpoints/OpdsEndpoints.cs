...
public static class OpdsEndpoints
{
    public static void MapOpdsEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            [FromQuery] string? url, CancellationToken ct) =>
        {
            ...
            try
            {
                ...
                return Results.Ok(new
                {
                    ...
                    books = feed.Books.Select(b => new
                    {
                        title = b.Title,
                        author = b.Author,
                        summary = b.Summary,
                        summaryIsHtml = b.SummaryIsHtml,
                        coverHref = b.CoverHref,
            ...
            [FromBody] ImportRequest req, CancellationToken ct) =>
        {
            ...
            // The user picks one of the two import buttons in the UI; the
            // mode is mandatory on the request body. Default to AddOnly
            // when missing/garbled rather than spinning up an AI pass the
            // user didn't ask for.
            var mode = ParseMode(req.Mode);
            // AI master-switch: silently downgrade to AddOnly when admin
            // turned the feature off, so a stale frontend doesn't 500 the
            // user mid-import.
            if (mode == OpdsImportMode.AddAndRunAi && !appS.AiEnabled)
                mode = OpdsImportMode.AddOnly;

            // Prime the editor repo + read EPUB metadata in one call.
    ...
    public sealed record SourceRequest(
        ...
        string? Username,
        string? Password);
    public sealed record ImportRequest(string Href, string? Title, string? Mode);

    private static OpdsImportMode ParseMode(string? raw) =>
        Enum.TryParse<OpdsImportMode>(raw, ignoreCase: true, out var m)
            ? m
            : OpdsImportMode.AddOnly;
