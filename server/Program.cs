...
// Redirect to /setup when no admin exists yet. Skips setup + auth endpoints
// so the setup flow itself can proceed without a chicken-and-egg problem.
app.Use(async (ctx, next) =>
{
    var path = ctx.Request.Path.Value ?? "";
    var isSetupOrAuth = path.StartsWith("/api/setup", StringComparison.OrdinalIgnoreCase)
                     || path.StartsWith("/api/auth",  StringComparison.OrdinalIgnoreCase)
                     || path.StartsWith("/setup",     StringComparison.OrdinalIgnoreCase);
    if (!isSetupOrAuth)
    {
        var users = ctx.RequestServices.GetRequiredService<UserManager<AppUser>>();
        ...
        if (admins.Count == 0)
        {
            if (path.StartsWith("/api/", StringComparison.OrdinalIgnoreCase))
            {
                ...
                await ctx.Response.WriteAsJsonAsync(new { setupNeeded = true });
                return;
            }
